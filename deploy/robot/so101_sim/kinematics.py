...
def lerobot_IK(q_now, target_pose, robot):
    ...
    Uses multiple attempts with different parameters and seeds to maximize success rate.
    ...
    # Try 1: Standard LM with current position as seed (fast, usually works for small deltas)
    sol = robot.ikine_LM(
        Tep=T,
        q0=q_now,
        ilimit=30,   # More iterations
        slimit=10,   # More search attempts
        tol=1e-3     # Reasonable tolerance
    )
    if sol.success:
        return sol.q, True
    
    # Try 2: Looser tolerance (for harder cases)
    sol = robot.ikine_LM(
        Tep=T,
        q0=q_now,
        ilimit=50,
        slimit=20,
        tol=5e-3     # Looser tolerance
    )
    if sol.success:
        return sol.q, True
    
    # Try 3: Random perturbation of seed (escape local minima)
    for _ in range(3):
