...
class HyperArguments:
    robot_config: str="deploy/robot/conf/dummy.yaml"
    publish_rate: int=20
    sensing_rate: int=50
    # ############## model  ################
...
def parse_param():
    ...
    for arg in unknown_args:
        if arg.startswith("--"):
            ...
            else: value = True  # If no value is specified (e.g., --flag), default to True
...
def make_robot(robot_cfg: Dict, args):
    ...
    robot_params = robot_cfg.get('params', {})

    robot = RobotCls(**robot_params, extra_args=args)
    # connect to robot
    retry_counts = 1
        ...
        while robot.is_running():
            ...
            if obs:
                ...
                if not observation_queue.full():
                    ...
                    try: observation_queue.get_nowait()
...
if __name__=='__main__':
    ...
    inference_thread = threading.Thread(target=inference_producer, args=(policy, normalizers, observation_queue, action_queue, args))
