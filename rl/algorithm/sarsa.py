...
class SARSAAlgorithm(FullRolloutUpdates, BaseRLAlgorithm):
    def __init__(
        ...
        *,
        ...
        super().__init__(
            required_capabilities=("action", "sarsa"),
        ...
        if "next_action" in result:
            ...
            for index, item in enumerate(items):
                if item.terminated:
                    ...
                    next_action = discrete_actions(
                        (items[index + 1],), device=q_values.device
                    )
                    derived.append(int(next_action[0]))
                else:
                    raise KeyError(
                        "SARSA needs next_action for a truncated or incomplete rollout tail"
                    )
            next_actions = torch.as_tensor(
                derived, device=q_values.device, dtype=torch.long
            )
